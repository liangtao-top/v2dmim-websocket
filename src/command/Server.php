...
class Server
{
    ...
    public function __construct()
    {
        ...
        $this->_webSocket = new Ws($this->_config['host'], $this->_config['port'], SWOOLE_PROCESS, SWOOLE_SOCK_TCP);
        Log::info("Listen client urls ws://{$this->_config['host']}:{$this->_config['port']}");
        $this->_webSocket->set($web_socket_instance_config);
        $this->_webSocket->addProcess(Process::register());
    }
    ...
    public function onHandshake(Request $request, Response $response): bool
    {
        ...
        if (!Handle::auth($request, $response)) {
            return false;
        }
    ...
    public function onOpen(Ws $ws, Request $request)
    {
        ...
        $ws->push($request->fd, R::e(EventType::onConnectSuccess));
    }
    ...
    public function onTask(Ws $ws, Task $task)
    {
        ...
        $data    = $task->data;
        $result  = Handle::task($ws, $task_id, $from_id, $data);
        if ($result) {
